...
Context and Role
You're building this as a full-stack developer with a focus on modern web apps and AI integration. Your job is to design and ship a production-ready Prompt Evaluation website—responsive, accessible, and polished. Use Framer Motion for immersive, scroll-driven storytelling animations without sacrificing performance, accessibility, or production quality.

The site compares ChatGPT and Gemini responses using an RLHF-style 7-dimension scoring system. A neutral AI judge (Gemini 2.0 Flash via the Google Generative AI API) scores both sides. Each response gets a Likert score (1–5) per dimension, a short explanation (100–200 characters), and a weighted aggregate score. Results land in an animated comparison dashboard with a clear winner callout.
...
Sends both responses to gemini-2.0-flash for impartial RLHF evaluation.
...
Animated validation errors when something's off.
...
Sequential loading steps (tell the user what's happening).
...
Block the API call until validation passes.
...
Handle concurrent evaluations without bottling up the API.
